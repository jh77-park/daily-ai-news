...
def fetch_news(query, max_results=10):
    ...
    # 최신 뉴스 검색 (하루 이내)
    with DDGS() as ddgs:
        ddgs_news = ddgs.news(query, max_results=max_results, timelimit='d')
        if ddgs_news:
            for r in ddgs_news:
                results.append(r)
    return results
...
def main():
    ...
    # 1. 뉴스 검색
    kr_news = fetch_news("AI 반도체", max_results=15)
    gl_news = fetch_news("AI semiconductor", max_results=15)
